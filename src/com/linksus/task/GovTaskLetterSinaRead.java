 ...
 * 读私信任务入口线程
 */
public class GovTaskLetterSinaRead extends BaseTask{
	...
	public void cal(){
		...
		//启动线程处理sina返回的私信
		Thread thread = new Thread(new GovLetterReadDealThread());
		thread.start();
		while (true){// 任务一直执行
			try{
				List<LinksusAppaccount> accounts = accountService
						.getLinksusGovAppaccountListWithMsgId(linksusAppaccount);
	...
	public static void main(String[] args) throws Exception{
		GovTaskLetterSinaRead letter = new GovTaskLetterSinaRead();
		Thread thread = new Thread(new GovLetterReadDealThread());
		thread.start();
		List accountList = new ArrayList();
		LinksusAppaccount account = new LinksusAppaccount();
		account.setId(10000L);
		account.setInstitutionId(10000L);
